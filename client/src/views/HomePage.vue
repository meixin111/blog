...
const loadBlogs = async (page = 0) => {
  ...
  // 只有当categoryId大于0时才传递
  if (pageInfo.categoryId > 0) {
    params.categoryId = pageInfo.categoryId;
  }
  // categoryId=0时不传递，后端就会查询所有文章

  console.log("请求参数:", params);

  let res = await axios.get("/blog/search", { params });
  ...
  pageInfo.pageCount = Math.ceil(pageInfo.count / pageInfo.pagesize); // 修复分页计算
  console.log(res);
};
</script>
